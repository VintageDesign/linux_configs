...
template <class T, class B> class BaseCalss : public AClass<T> {
} template <class T, class B<E>> class BaseCalss : public AClass<T> {
}

Aclass<T>
    aClass = bClass->getThing();

for (int i = 0; i < 12; i++) {
}

template <T> bool operator<double><(int a, int b) { return b > a; }
